...
#     def inner():
#         print(m+n)
#     return inner  #返回的是函数名inner,不是inner()
# outer(10)()
# ot=outer(10)
# ot()
...
# #第三次调用
# re(30)
#当外函数被调用一次，通过闭包机制创建了一个内函数(该函数捕获了外函数的变量),虽然之后每次调用这个内函数都会新建一个函数调用帧(执行后销毁),但被捕获的变量只有一份，所有内函数的调用共享同一个捕获变量
#理解：即使re被调用多次，m(闭包捕获的变量)始终是那一份，保存在inner函数__closure__属性中，不会因为内函数的多次调用而被复制

#函数引用
# def func():
#     print(10)
# print(func)     #函数名里面保存了函数所在位置的引用
# #id()判断两个变量是否是同一个值的引用
# # a=1   #a是一个变量名，存的是1这个数值所在的内存地址，意思是a保存了数值1的引用
# fun=func
# fun()   #通过引用调用函数
...
# def test(fn):
#     print('开始计算')
#     fn(3,4)   #fn等待接收的一个函数对象，3和4是给函数传的两个值
# test(func1)   #调用test函数，把func1作为参数传进去
#上面的代码改变了函数的调用方式

#装饰器：本质上就是一个闭包函数，在不改变原有代码的情况下添加新的功能
#条件：1.不修改原程序或函数的代码
#     2.不改变函数或程序的调用方法
