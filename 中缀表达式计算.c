...
/*
中缀表达式转前缀表达式（机算）
从右向左处理各个元素，直到开头。可能遇到三种情况：
1.遇到操作数。直接加入前缀表达式
2.遇到界限符。遇到")"直接入栈；遇到"("则依次弹出栈内运算符并加入前缀表达式，直到弹出")"为止。注意：")"不加入前缀表达式。
3.遇到运算符。依次弹出栈中优先级高于（不包含等于）当前运算符的所有运算符，并加入前缀表达式，若碰到")"或栈空则停止。之后再把当前运算符入栈。
按上述方法处理完所有字符后，将栈中剩余运算符依次弹出，并加入前缀表达式。
最后将得到的结果反转，即为最终的前缀表达式。

前缀表达式的计算（机算）
用栈实现前缀表达式的计算：
1.从右往左扫描下一个元素，直到处理完所有元素
2.若扫描到操作数，则压入栈，并回到步骤1，否则执行步骤3
3.若扫描到运算符，则弹出两个栈顶元素（先出栈的是左操作数），执行相应运算，运算结果压回栈顶，回到步骤1
最后栈中剩下的唯一元素就是计算结果
*/
#include <stdio.h>
#include <stdlib.h>
